...
class SemanticInjector:
                    ...
                    if match:
                        ...
                        if edge_type == "extension":
                            source, target = (
                                target,
                                source,
                            )  # Swap for inheritance direction
    ...
    def _calculate_animation_order(
        self, edges: List[EdgeSemantics]
    ) -> List[EdgeSemantics]:
        ...
        edge_graph: Dict[str, List[int]] = defaultdict(
            list
        )  # node -> list of outgoing edge indices
        edge_targets: Dict[str, List[int]] = defaultdict(
            list
        )  # node -> list of incoming edge indices
        ...
        if not start_nodes:
            ...
            start_nodes = [
                node for node in all_nodes if len(edge_targets[node]) == min_incoming
            ]
            ...
            for edge_idx in outgoing_edge_indices:
                if not edge_order_assigned[edge_idx]:
                    ...
                    if target_node not in visited_nodes:
                        ...
                        all_incoming_processed = all(
                            edge_order_assigned[idx] for idx in incoming_edges
                        )
    ...
    def _add_semantic_attributes(
        self, path_element: ET.Element, semantics: EdgeSemantics
    ) -> None:
